...
var all = time.Duration(0) * time.Second
var count = 0
var bcs_index = 0
...
func chaincodeInvokeOrQuery(cmd *cobra.Command, invoke bool, cf *ChaincodeCmdFactory) (err error) {
	...
	file, err := os.Open("input_number.txt")   //打开
	if err != nil { fmt.Println(err); return  }
	defer file.Close() //关闭
 
	line := bufio.NewReader(file)
	content, _, err := line.ReadLine()
	input_number, err := strconv.Atoi(string(content))
	
	fmt.Println("input_number = ", input_number)

	for i := 0; i < input_number; i++ {
			
		my_arg1 := "asset" + strconv.Itoa(rand.Intn(5000))
		my_arg2 := "owner" + strconv.Itoa(rand.Intn(5000))
		
		//fmt.Println("arg1 = " + my_arg1)
		//fmt.Println("arg2 = " + my_arg2)
		
		spec.Input.Args[1] = []byte(my_arg1)
		spec.Input.Args[2] = []byte(my_arg2)
	...
	//time.Sleep(time.Duration(1000*(round+5))*time.Millisecond)
	logger.Warningf("all : %d , count : %d,  per : %d", all.Microseconds(), count, ((int)(all.Microseconds()))/count)
	
	filename := "latency_result.txt"
 
    latency_result_f, err := os.OpenFile(filename, os.O_RDWR|os.O_CREATE , 0755)
    if err != nil {
        fmt.Println(err)
        return err
    }
    
    latency_result_f.WriteString(fmt.Sprintf("all : %d , count : %d,  per : %d", all.Microseconds(), count, ((int)(all.Microseconds()))/count))
    latency_result_f.Close()
...
func InitCmdFactory(cmdName string, isEndorserRequired, isOrdererRequired bool, cryptoProvider bccsp.BCCSP) (*ChaincodeCmdFactory, error) {
	...
	if isOrdererRequired {
		file, err := os.Open("input_orderer_endpoints.txt")   //打开
		...
		for {
			...
			address := buf.Text()
			fmt.Println(address)
			broadcastClient, err := common.GetBroadcastClientFnc(address)
			if err != nil {
...
func ChaincodeInvokeOrQuery(
	...
	prop, _, err := protoutil.CreateChaincodeProposalWithTxIDAndTransient(pcommon.HeaderType_ENDORSER_TRANSACTION, cID, invocation, creator, txID, tMap)
	if err != nil {
	...
	if invoke {
		if proposalResp != nil {
			...
			now := time.Now()			
			//fmt.Println("bcs_index=",bcs_index)
			err = bcs[bcs_index].Send(env)
			bcs_index ++
			bcs_index = bcs_index%len(bcs)
			t1 := time.Since(now)
			all = all + t1
			count = count + 1
