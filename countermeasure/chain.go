...
func sufficientCpuResources() bool {
...
func getCpuPercent(millisecond float64) float64 {
	percent, _:= cpu.Percent(time.Duration(millisecond)*time.Millisecond, false)
	return percent[0]
}
...
func existExceedThreshold(l *list.List, threshold float64) bool{
	for i := l.Front(); i != nil; i = i.Next() {
		if i.Value.(float64) >= threshold {
			return true
		}
	}
	return false
...
func (c *Chain) run() {
	...
	becomeLeader := func() (chan<- *common.Block, context.CancelFunc) {
		...
		go func(ctx context.Context, ch chan<- struct{}) {
			const interval float64 = 500  //millisecond
			const highCPUTime float64 = 10	//second
			
			const highCPUCountMax int = int(highCPUTime*1000/interval)
			var highCPUCount int = 0
			
			const threshold float64 = 90
			
			weight := math.Pow(math.E, -interval/1000)
			// var array1 = []float64{10,110,110,210,210,247,310,410,292,410,307}
			//var array1 = []float64{10,133,70,219,113,161,963,2137,4196}
			
			var t_prio float64 = 0
			var t_curr float64 = 0
			
			var tcp_prio float64 = getTcpNumber()
			var tcp_curr float64 = 0
			
			var v_curr float64 = 0
			
			var t_min float64 = 100
			var t_max float64 = 500
			var p_max float64 = 0.8
			var p_curr float64 = 0
			
			cpu_list := list.New()
			// f, err3 := os.OpenFile("orderer_transfer.log", os.O_APPEND|os.O_WRONLY, 0666)
			// if err3 != nil{
			// 	panic(err3)
			// }
			// defer f.Close()
			for {
				...
				cpu_curr := getCpuPercent(interval)
		
				if cpu_curr >= threshold{
					highCPUCount ++
				} else {
					highCPUCount = 0
				}
				
				if highCPUCount >= highCPUCountMax{
					ch <- struct{}{}
					// f.WriteString("found in case of highCPUCount >= highCPUCountMax\n")
					// f.Sync()
					return
				}
				
				tcp_curr = getTcpNumber()
				v_curr = tcp_curr - tcp_prio
				
				tcp_prio = tcp_curr
				t_curr = weight * t_prio + (1-weight) * v_curr
				t_prio = t_curr
				if t_curr <= t_min {
					p_curr = 0
				} else if t_curr >= t_max {
					p_curr = 1
				} else {
					p_curr = p_max * (t_curr-t_min)/(t_max-t_min)
				}
				
				if p_curr > 0{
					cpu_list.PushBack(cpu_curr)
				} else {
					cpu_list = list.New()
				}
				...
				random := rand.Float64()
				// f.WriteString("cpu_curr:" + strconv.FormatFloat(cpu_curr, 'E', -1, 64) + "  v_curr:" + strconv.FormatFloat(v_curr, 'E', -1, 64) +
				//  "  t_curr:" + strconv.FormatFloat(t_curr, 'E', -1, 64) + "  p_curr:" + strconv.FormatFloat(p_curr, 'E', -1, 64) + 
				// "  random:" + strconv.FormatFloat(random, 'E', -1, 64) + "\n")
				// f.Sync()
				
				if random <= p_curr && existExceedThreshold(cpu_list, threshold){
					ch <- struct{}{}
					// f.WriteString("found in case of random <= p_curr\n")
					// f.Sync()
					return
				}
				
				// if !sufficientCpuResources() {
				// 	count ++
				// 	if count >= 5 {
				// 		ch <- struct{}{}
				// 		// f.WriteString(time.Now().String() + " count >= 5 : " + strconv.Itoa(count) + "\n")
				// 		// f.Sync()
				// 		return
				// 	}
				// } else {
				// 	count = 0
				// }
				// f.WriteString(time.Now().String() + " count = " + strconv.Itoa(count) + "\n")
				// f.Sync()
				select {
				case <-ctx.Done():
					c.logger.Debugf("Quit transfer")
					return
				default:
				}
			}
	...
	for {
		...
		case <-tC:
			if c.raftID == c.lastKnownLeader {
				lead := atomic.LoadUint64(&c.lastKnownLeader)
				c.Node.abdicateLeader(lead)
				c.logger.Infof("Leader transfer because of high CPU utilization rate")
				// f, err3 := os.OpenFile("orderer_transfer.log", os.O_APPEND|os.O_WRONLY, 0666)
				// if err3 != nil{
				// 	panic(err3)
				// }
				// defer f.Close()
				// f.WriteString(time.Now().String() + " Leader transfer because of high CPU utilization rate\n")
				// f.Sync()
			} else {
				// f, err3 := os.OpenFile("orderer_transfer.log", os.O_APPEND|os.O_WRONLY, 0666)
				// if err3 != nil{
				...
				// }
				// defer f.Close()
				// f.WriteString(time.Now().String() + " Follower cannot transfer because of high CPU utilization rate\n")
				// f.Sync()
				c.logger.Infof("Follower cannot transfer because of high CPU utilization rate")
			}
